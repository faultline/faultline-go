...
func TestGobrake(t *testing.T) {
	...
	RunSpecs(t, "faultline")
}

var _ = Describe("Notifier", func() {
	var notifier *faultline.Notifier
	var sentNotice *faultline.Notice
	...
	BeforeEach(func() {
		handler := func(w http.ResponseWriter, req *http.Request) {
			...
			sentNotice = new(faultline.Notice)
	...
	It("reports error and backtrace", func() {
		...
		frame := e.Backtrace[0]
		Expect(frame.File).To(Equal("/GOPATH/github.com/faultline/faultline-go/faultline/notifier_test.go"))
	...
	It("reports error and backtrace when error is created with pkg/errors", func() {
		...
		frame := e.Backtrace[0]
		Expect(frame.File).To(Equal("/GOPATH/github.com/faultline/faultline-go/internal/testpkg1/testhelper.go"))
	...
	It("reports context using SetContext", func() {
		notifier.AddFilter(func(notice *faultline.Notice) *faultline.Notice {
	...
	It("collects and reports some context", func() {
		...
		Expect(sentNotice.Context["rootDirectory"]).To(Equal(rootDirectory))
		Expect(sentNotice.Context["gopath"]).To(Equal(gopath))
		Expect(sentNotice.Context["component"]).To(Equal("github.com/faultline/faultline-go/faultline_test"))
...
var _ = Describe("rate limiting", func() {
	...
	It("pauses notifier", func() {
		...
		for i := 0; i < 3; i++ {
			...
			Expect(err).To(MatchError("faultline: IP is rate limited"))
		}
